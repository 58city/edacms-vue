  ...
  <div id="siteInfo">
    <el-form ref="siteInfoForm" :model="siteInfo" :rules="rules" label-width="92px">
      <el-form-item label="网站标题：" prop="title">
        <el-input v-model="siteInfo.title" type="text" @blur="formBlur"></el-input>
      </el-form-item>
      <el-form-item label="页面主题：" prop="theme" :error="themeError?'主题读取失败，请检查 /public/themes/ 下主题目录完整性':''">
        <el-select v-model="siteInfo.theme">
          <el-option v-for="(item,index) in themes" :key="index" :label="item.name" :value="item.directory" ></el-option>
        </el-select>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules:{
        title:[
          { required: true, message: '网站标题为必填项', trigger: 'blur' }
        ],
        theme:[
          { required: true, message: '页面主题为必填项', trigger:  ["blur",'change'] }
        ],
        email:[
