// 参数window：①对象转变量便于压缩、②减少作用域搜索层级
// 参数undefined：①避免IE6/7/8下undefined可以被重写的bug，防止在外部被修改
(function( window, undefined ) {

	// 慎用：严格模式，因为严格模式会导致一些不规范的用法报错
	// 例如：不使用var声明变量，函数中arguments.callee失效
	//"use strict";
	...
	readyList,
	// 将字符串的"undefined"赋值该变量
	// 因为IE9及其以下版本，如果xmlNode.method不存在是不等于undefined的，即`xmlNode.method == undefined`不成立
	// 但是`typeof xmlNode.method=='undefined'`却是成立的
	core_strundefined = typeof undefined,
	// Use the correct document accordingly with window argument (sandbox)
	location = window.location,
	document = window.document,
	docElem = document.documentElement,
	// 防冲突：用于记录保存上一个拥有jQuery和$控制权的库
	// 当调用noConflict方法时把jQuery和$的控制权让出去
	...
	// 对外暴露的jQuery对象，`$()`调用的即是该函数
	// 我们常用的一些jQuery方法就是位于该jQuery对象的方法中
	// 但是如果为了使用该对象的原型方法每次都在外部new $().addClass()会比较繁琐
	// 所以jQuery构造函数内部返回了一个新的实例对象，并在下面让这个实例对象的原型指向jQuery对象的原型
	...
	jQuery.fn = jQuery.prototype = {
		...
		init: function( selector, context, rootjQuery ) {
			...
			/*
			 * 处理：$('<li>')、$('<li>内容</li>')、$('<li>内容')、$('#box')、$('.cmp')、$('div')
			 */ 
			if ( typeof selector === "string" ) {
				if ( selector.charAt(0) === "<" && selector.charAt( selector.length - 1 ) === ">" && selector.length >= 3 ) {
					// $('<li>')          ==>[null,'<li>',null]
					// $('<li>内容</li>') ==>[null,<li>内容</li>,null]
					match = [ null, selector, null ];
				} else {
					// /^(?:\s*(<[\w\W]+>)[^>]*|#([\w-]*))$/
					// $('.cpm')     ==>null
					// $('<li>内容') ==>['<li>内容','<li>',undefined]
		...
		// ==> ② document.readyState=='complete' >> jQuery.ready()
		// ==> ③ 否则监听DOMContentLoaded | load  >> complete() >> 移除监听+ jQuery.ready()
	...
	/*
	 * 对象拷贝继承，功能如下：
	 * ① 扩展静态方法：$.extend({fn1,fn2})
	 * ② 扩展实例方法：$.fn.extend({fn1,fn2})
	 * ③ 对象的浅拷贝：$.extend({},{a:123,b:{title:'hh'}})
	 * ④ 对象的深拷贝：$.extend(true,{},{a:123,b:{title:'hh'}})
	 */
		...
		for ( ; i < length; i++ ) {
			if ( (options = arguments[ i ]) != null ) {
				for ( name in options ) {
					...
					copy = options[ name ];
					// 防止对象自身的循环引用(浅拷贝)
					// let a={age: 20, height: 107}
					// $.extend(a,{title:a})
					if ( target === copy ) {
						continue;
					}
					// 深拷贝
					// 第一次：$.extend(true,{title:{level1:'h1h1h1'}},{title:{level2:'h2h2h2'}})
					// 第二次：$.extend(true,{level1:'h1h1h1'},{level2:'h2h2h2'})
	...
	jQuery.extend({
		...
		ready: function( wait ) {
			// 调用$.holdReady(false)取消延迟状态时，如果为0则执行下面的“加载完成回调”
			// $.holdReady(true)
			// $.getScripts('a.js',function(){ $.holdReady(false) })
			// $(function(){})
			if ( wait === true ? --jQuery.readyWait : jQuery.isReady ) {
				return;
			}
			jQuery.isReady = true;
			// DOM加载完成调用jQuery.ready()时，如果还处于holdReady状态未取消，则返回
			// $.holdReady(true)
			// $(function(){})
			if ( wait !== true && --jQuery.readyWait > 0 ) {
				return;
			}
			// 调用readyList延迟对象上的所有done回调，并设置上下文和传参
			// $(function(args){
			//   console.log(this) ==> document
			//   console.log(args) ==> jQuery类
			// })
			readyList.resolveWith( document, [ jQuery ] );
			// 触发第三种DOM加载完成方法
			// $(function(){})
	...
	jQuery.ready.promise = function( obj ) {
		if ( !readyList ) {
			...
			if ( document.readyState === "complete" ) {
				// IE浏览器会在文档加载完成前提前触发complete，所以需要适当setTimeout，不写默认window是15ms,ios是10ms
