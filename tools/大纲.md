...
server=net.createServer(function(socket){
    ...
    // 		console.log(buffer.toString())
	   //  	let httpHeaders=buffer.toString().split('\r\n').slice(0,-2).slice(1)
	   //  	let headers={}
	   //  	httpHeaders.forEach(function(item,index){
...
<script>
    (async ()=>{
        let ws=new Socket('ws://localhost:8888')
        while(!ws.end){
            document.getElementById('btn').onclick=function(){
                ws.send('sss')
            }
            let msg=await ws.getMsg()
            console.log(msg)
        }
    })()
</script>
...
<Buffer 81 83 1f 2b f4 9c 6c 58 87>

 81                       83                     掩码（1f 2b f4 9c）                                            数据(加密后)
 <u>1000 0001</u>         <u>1 0000011</u>      00011111 00101011 11110100 10011100     01101100  01011000   10000111                   

数据掩码    ：<u>0001 1111</u>  <u>0010 1011</u>   <u>1111 0100</u> 1001 1100

加密的数据：<u>0110 1100</u>  <u>0101 1000</u>   1000 0111

原始数据    ：0111 0011  0111 0011   0111 0011
...
+-------------------------------+-------------------------------+
|Extended payload len continued | Masking-key, if MASK set to 1 |
+-------------------------------+-------------------------------+
...
性能高（比操作DOM要高至少几十倍）、修改图形做不到（需要擦除重绘）
...
<script>
    ...
    gd.lineTo(300, 200);
    gd.lineWidth=10;
    gd.strokeStyle='rgba(255,255,0,.4)';
    gd.stroke();
</script>
```
...
```html
<canvas id="oc" width="800" height="400"></canvas>
<script src="common.js"></script>
...
class Sprite{
    ...
    render(){
        assert(false,`请重写${this.constructor.name}中的render方法`)
    }
}
```
...
<body>
	...
	<script src="rect.js"></script>
	<script>
		var container=new Canvas('#oc')
		var r1=container.add('rect',0,0,50,50)
		r1.x=100
		var r2=container.add('rect',150,200,50,50)
		container.render()
	</script>
</body>
</html>
...
class Canvas{
	constructor(canvas_el){
		assert(canvas_el,'请传入要管理的canvas元素')
		this._canvas=this._getRoot(canvas_el)
		this._context=this._canvas.getContext('2d')
		this._shape_class_map={
			rect:Rect
		}
		this._shape_collection=[]
	}
	_getRoot(el){
		if(typeof el=='string'){
			el=document.querySelector(el)
			assert(el,'您传入的canvas元素不存在')
			assert(el.nodeName.toLowerCase()=='canvas','您传入的元素不是canvas')
			return el
		}else if(el instanceof HTMLElement){
			assert(el.nodeName.toLowerCase()=='canvas','您传入的元素不是canvas')
			return el
		}else{
			assert(false,'您检查您传入的参数')
		}
	}
	add(shape,...args){
		shape=new this._shape_class_map[shape](...args)
		this._shape_collection.push(shape)
		return shape
	}
	render(){
		this._context.clearRect(0,0,this._canvas.width,this._canvas.height)
		this._shape_collection.forEach((item,index)=>{
...
<body>
	...
	<script src="rect.js"></script>
	<script>
		var container=new Container('#oc')
        # 第一步： 初始传值
		var r1=container.add('rect',2,4,50,50)
		var r2=container.add('rect',20,40,30,150)
		setInterval(function(){
			r1.x++
			r2.y++
		},1000/60)		
...
class Container{
	...
	add(shape,...args){
		shape=new this._shape_class_map[shape](this,...args)
		this._shape_collection.push(shape)
		return shape._data
	}
    # 第四步： 触发update方法，调用render
	update(){
...
class Sprite{
	constructor(container){
		assert(container,'传给sprite的container不存在')
		assert(container instanceof Container,'错误的container实例')
		this._container=container
		let _this=this
		this._data=new Proxy({x:0,y:0,w:0,h:0,bgColor:'#000'},{
			get(data,name){
				return data[name]
			},
            # 第三步：监测属性的变化，调用容器的update方法
			set(data,name,value){
				data[name]=value
				_this._container.update()
				return true
			}
		})
	}
...
class Rect extends Sprite{
    ...
    # 第五步：调用最终的渲染方法
	render(context){
		context.fillStyle=this._data.bgColor
		context.fillRect(this._data.x,this._data.y,this._data.w,this._data.h)
	}
}
```
...
第一步：使用事件队列的on方法，给一个创建的形状添加事件，并给其做一个标记，过程中存在两个问题

- 如何在外界调用属于形状对象继承事件队列而来的on方法，因为add时返回的是一个Proxy
- 如何做标记

第二步：事件队列里已经注册了事件，如何进行emit触发

- 给整体添加一个事件
- 遍历所有的形状，判断属性中是否存在事件标记，存在则形状.emit触发当前对象的所有该事件
- 同时要检测事件触发的范围是否位于该形状上
...
function assert(exp,msg){
	if(!exp){
		throw new Error(msg||'invalid parameters')
	}
}
```
...
class Container{
	...
	_initEvent(){
		this._canvas.onclick=ev=>{
			this._shape_collection.forEach(shape=>{
		        if(shape._data.clicked){
		          	let x=ev.offsetX,y=ev.offsetY;
		          	if(shape._isIn(x, y)){
		            	shape.emit('click', x, y);
		          	}
		        }
      		});
		}
	...
	add(shape,...args){
		shape=new this._shape_class_map[shape](this,...args)
		this._shape_collection.push(shape)
		// 此时如果直接使用this.on赋值，则on内的this为shape._data代表的Proxy,其内没有pipes导致报错
		// bind把on内的this指向改为当前形状对象，则on或emit时就会操作当前形状对象间接继承而来的pipes
		
		// 注意：在给当前形状对象的_data对象添加on和emit引用时，添加的是当前形状重写的on方法
		// 因为在重写的on方法中，会给当前形状对象做一个被添加了事件的标志
		shape._data.on=shape.on.bind(shape)
		shape._data.emit=shape.emit.bind(shape)
		return shape._data
	}
	update(){
		clearTimeout(this._timer)
...
class Sprite extends Pipe{
	constructor(container){
		...
		this._container=container
		let _this=this
		this._data=new Proxy({x:0,y:0,w:0,h:0},{
			get(data,name){
				return data[name]
			},
			set(data,name,value){
	...
	_isIn(){
		assert(false,`请重写${this.constructor.name}中的_isIn方法`)
	}
}
```
...
class Rect extends Sprite{
	...
	render(context){
		context.fillStyle=this._data.bgColor||'#00f'
		context.fillRect(this._data.x,this._data.y,this._data.w,this._data.h)
	}
	_isIn(x,y){
        return x>this._data.x&&x<(this._data.x+this._data.w)&&
               y>this._data.y&&y<(this._data.y+this._data.h)
	}
}
```
