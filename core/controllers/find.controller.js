...
module.exports = function (req, res, next) {
	req.checkQuery({
		'pd':{
			notEmpty: {
				errorMessage: 'pd cannot be empty'
			}
		},
		'page': {
			optional: true,
	...
	categoriesService.one({name:req.query.pd,type: 'channel'}, function (err, category) {
		...
		if(!req.query.action){
			async.parallel({
				...
				list: function (callback) {
					...
					// 	_id: category._id
					// };
					// if (_.get(category, 'mixed.pageSize')) {
					// 	query.pageSize = category.mixed.pageSize;
					// }
					// if (req.query.page) {
					// 	query.currentPage = parseInt(req.query.page);
					// }
				...
				localReadingTotal: function (callback) {
					listsService.reading({ name: req.query.pd }, callback);
				},
				localReadingDay: function (callback) {
					listsService.reading({ name: req.query.pd, sort: '-reading.day' }, callback);
				},
				localReadingWeek: function (callback) {
					listsService.reading({ name: req.query.pd, sort: '-reading.week' }, callback);
				},
				localReadingMonth: function (callback) {
					listsService.reading({ name: req.query.pd, sort: '-reading.month' }, callback);
				}
