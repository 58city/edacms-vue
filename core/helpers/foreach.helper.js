...
module.exports = function (context, options) {
	...
	function execIteration(context,field, index, last) {
		if (data) {
			data.key = field;
			data.index = index;
			data.number = index + 1;
	...
	function iterateCollection(context) {
		var count = 1,
			current = 1;
		if(options.hash.sort){
			if(options.hash.sort.indexOf('-')==0){
				context = _.sortBy(context, options.hash.sort.substring(1)).reverse();
			}else{
				context = _.sortBy(context, options.hash.sort);
			}
		}
		_.each(context, function (item, key) {
			...
			if (current <= to) {
				execIteration(context,key, current - 1, current === to);
			}
			count += 1;
			current += 1;
		});
