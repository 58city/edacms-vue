	...
	// 上传
		...
		async.auto({
			...
			formParse: function (callback) {
				...
				form.encoding = 'utf-8';
				form.uploadDir = 'tmp';
				form.keepExtensions = false;           // 不保留扩展名
				form.maxFieldsSize = 10 * 1024 * 1024; // 10MB
				form.multiples = false;                // 单文件上传            
			...
			// 移动文件
			moveFile: ['formParse', 'saveModel', 'mkdirFolder', function (callback, results) {
				var tmp_name=path.join(__dirname, '../../' + results.formParse.path);
				var new_name=path.join(__dirname, results.mkdirFolder + '/' + results.saveModel.fileName);
				fs.rename(tmp_name, new_name, function (err) {
					if (err) {
						err.type = 'system';
						return callback(err);
					}
			...
			// 压缩大图/生成小图
			generateSmallImg:['moveFile',function(callback,results){
				if(results.formParse.type.startsWith('image')){
					var originalImg=results.moveFile;
					var smallImg=path.join(__dirname, results.mkdirFolder + '/small-' + results.saveModel.fileName);
					gm(originalImg).quality(80).write(originalImg,function(err){
						if (err) {
							err.type = 'system';
							return callback(err);
						}
					...
					gm(originalImg).resize(110).write(smallImg,function(err){
						if (err) {
							err.type = 'system';
							return callback(err);
						}
