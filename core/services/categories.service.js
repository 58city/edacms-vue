			...
			.exec(function (err, categories) {
				...
				categories = _.map(categories, function (category) {
					if (category.type === 'page' && !_.isEmpty(category.mixed.pageMedia)) {
						// console.log(category.mixed.pageMedia)
						// [{"_id":"5da4902e9954e518b87ea24b","fileName":"2.gif","date":"2019-10-14T15:11:42.082Z"}]
						var mediaSrc = _.map(category.mixed.pageMedia, 'src');
						// console.log(mediaSrc)
						// [/media/201910/5da4902e9954e518b87ea24b/2.gif]
					}
					...
					if (category.type === 'page' && !_.isEmpty(category.mixed.pageMedia)) {
						...
						// console.log(category.mixed.pageMedia)
						// [{"_id":"5da4902e9954e518b87ea24b","fileName":"2.gif","date":"2019-10-14T15:11:42.082Z", src: '/media/201910/5da4902e9954e518b87ea24b/2.gif' } ]
					}
					// 删除非单页分类的 pageMedia
					if (category.type !== 'page' && category.mixed) delete category.mixed.pageMedia; 
					return category;
			...
			function loop(nodes) {
				return _.map(nodes, function (category) {
					if (category.path && category.path !== '/') {
						...
						// current ==> /series/china
						// /^\/series/.test(current)         ==> true
						// /^\/series\/china/.test(current)  ==> true
...
exports.one = function (options, callback) {
	...
	exports.all(function (err, categories) {
		...
		var category = _(categories).map(function (category) {
		...
		if (category) {
			...
			// pick(object, [props])创建一个从 object 中选中的属性的对象
			// path ==> /series/china ==> ['','series','china'] ==> ['series'] ==> Object
			// Object ==> { 
			// 	   _id: '5da29dc78b72d016745ca31b',name: '电视剧',path: '/series',keywords: '',description: '',mixed: {},
			//     views: { layout: 'layout-default', channel: 'channel-default' },isShow: true,sort: 0,type: 'channel' 
			// }      ==> { name: '电视剧', path: '/series' } ==> [ { name: '综艺', path: '/show' } ]
			category.breadcrumb = _(path.split('/')).tail().initial().map(function (prePath) {
				var temp=_.find(categories, function (category) {
					var regex = new RegExp('\/' + prePath + '$');
					return regex.test(category.path);
				})
				return _.pick(temp, ['name', 'path']);
			}).value();
 ...
 * 获取model
 */
exports.getColumnModel = function(options,callback){
	var query={type:'column'};
	if(options.path) query.path=options.path;
	categoriesModel
		.findOne(query)
		.select('model')
		...
		.lean()
		.exec(function(err,column_cat){
			if(err) {
				err.type = 'database';
				return callback(err);
			}
			callback(null,column_cat.model);
		});
}
/**
 * 保存分类
